...
class mtcnnface():
    def __init__(self, min_face_size=20.0, thresholds=(0.6, 0.7, 0.8), nms_thresholds=(0.7, 0.7, 0.7), device = 'cuda'):
        self.pnet, self.rnet, self.onet = PNet().eval().to(device), RNet().eval().to(device), ONet().eval().to(device)
